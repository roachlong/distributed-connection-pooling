...
LOG_FILE = os.getenv("LOG_FILE", "logs/copy_obs_data.log")
...
def get_connection():
    ...
    # Retry connection establishment with exponential backoff (SSL errors are transient)
    max_retries = 3
    for attempt in range(max_retries):
        try:
            conn = psycopg.connect(DATABASE_URL, autocommit=False)
            # Enable access to crdb_internal tables (required for v26.1+)
            ...
            if attempt < max_retries - 1:
                ...
                log.warning(f"Connection failed (attempt {attempt+1}/{max_retries}), retrying in {delay}s: {e}")
                time.sleep(delay)
            else:
                log.error(f"Connection failed after {max_retries} attempts: {e}")
                raise
...
def backfill_contention(conn, run):
    ...
    # CockroachDB doesn't allow joining crdb_internal vtables in UPDATE FROM
    # Use a CTE workaround: materialize the virtual table data first
    sql = f"""
    WITH src AS MATERIALIZED (
      SELECT
        ...
        waiting_txn_fingerprint_id
      FROM crdb_internal.transaction_contention_events
      WHERE collection_ts >= now() - interval '{BACKFILL_WINDOW_HOURS} hours'
    )
